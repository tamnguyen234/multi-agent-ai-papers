...
@router.post("/run-summarizer-daily-top5", status_code=status.HTTP_202_ACCEPTED)
def run_summarizer_daily_top5(background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    """Trigger daily top 5 summarization in the background. Returns immediately."""
    if settings.APP_ENV.lower() != "development":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    def _run():
        ...
        _db = SessionLocal()
        try:
            DigestService.run_daily_digest_flow(_db)
            logger.info("Background digest flow completed.")
        except Exception as e:
            logger.error(f"Background digest flow failed: {e}", exc_info=True)
        finally:
            _db.close()

    background_tasks.add_task(_run)
    return {"status": "started", "message": "Digest job đang chạy nền. Làm mới trang sau 1-2 phút để xem kết quả."}
...
@router.post("/run-daily-digest-job", status_code=status.HTTP_202_ACCEPTED)
def run_daily_digest_job_endpoint(background_tasks: BackgroundTasks):
    """Trigger the daily digest job in the background. Returns immediately (202 Accepted)."""
    if settings.APP_ENV.lower() != "development":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    def _run():
        try:
            ...
            logger.info(f"Background daily digest job completed: {result}")
        except Exception as e:
            logger.error(f"Background daily digest job failed: {e}", exc_info=True)
