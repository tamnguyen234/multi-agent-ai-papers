...
2. **Backend Gateway (FastAPI + APScheduler)** (Cổng `8000`): Cổng điều phối trung tâm, quản lý định tuyến, xác thực JWT, lập lịch công việc (APScheduler) chạy lúc 2:00 AM hàng ngày. Đặc biệt, Backend tích hợp sẵn **Trend Model** (BERTopic/UMAP) và **Summarization Service** (nếu cấu hình) ở tầng nội bộ.
3. **Q&A Agent (LangChain + FAISS + Ollama)** (Cổng `8103`): Xây dựng cơ sở tri thức vector RAG từ file PDF của bài báo. Tích hợp với **Ollama portable** chạy embedding `nomic-embed-text` và model `qwen3.5:4b` offline để trả lời câu hỏi của người dùng.
4. **TTS Agent (VieNeu)** (Cổng `8104`): Chuyển đổi văn bản tóm tắt/chat sang audio tiếng Việt sử dụng voice Ngọc Linh (nữ) hoặc Gia Bảo (nam).
5. **Database & Storage (MySQL + Local File Storage)**: Lưu trữ 10 bảng dữ liệu MySQL đồng bộ qua **Alembic migrations** và lưu trữ file tĩnh (PDFs, audio, indexes) trong thư mục `data/`.
...
> [!NOTE]
> Theo cấu hình `.gitignore`, toàn bộ thư mục chứa tệp dữ liệu thật phát sinh (`data/paper_pdf/**`, `data/audio_abstract/**`, `data/audio_chat_message/**`, `data/indices_v2/**`) được loại trừ khỏi Git để tránh commit tệp nhị phân nặng lên kho chứa. Dữ liệu này được tạo và lưu trữ cục bộ trên máy chạy.
...
1. **Ollama không kết nối được (Cổng 11434)**:
   - Đảm bảo Ollama đã tải và chạy được model `qwen3.5:4b` và `nomic-embed-text` cục bộ.
...
4. **Xung đột cổng kết nối (Port Conflicts)**:
   - Nếu các cổng `8000`, `8103`, `8104` hoặc `5173` bị chiếm dụng bởi các tiến trình chạy ngầm cũ, hãy chạy:
     ```cmd
