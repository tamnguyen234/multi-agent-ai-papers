...
5. **Q&A Agent (LangChain + FAISS + Ollama)** (Cổng `8103`): Xây dựng cơ sở tri thức vector RAG từ file PDF của bài báo. Tích hợp với **Ollama portable** chạy embedding `nomic-embed-text` và model `qwen3.5:4b` offline để trả lời câu hỏi của người dùng.
