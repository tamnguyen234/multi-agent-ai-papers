...
class RealTTSEngine:
    ...
    def load_translation_model(self):
        ...
        Model: facebook/nllb-200-distilled-600M (600M params, supports 200 languages).
        """
        ...
        logger.info("Initializing NLLB-200 Translate model (facebook/nllb-200-distilled-600M)...")
        try:
            from transformers import AutoTokenizer, AutoModelForSeq2SeqLM
            model_name = "facebook/nllb-200-distilled-600M"
            device = "cpu"

            # NLLB does not take src_lang in the constructor — it is set per-call
            ...
            logger.info(f"NLLB-200 Translate model loaded successfully on {device}.")
            return self._translation_tokenizer, self._translation_model
        except Exception as e:
            self._translation_error = RuntimeError(f"Translation model load failed: {str(e)}")
            logger.error(f"Failed to load NLLB-200 Translate model: {str(e)}", exc_info=True)
            raise self._translation_error
    ...
    def translate_en_to_vi(self, text: str) -> str:
        ...
        # NllbTokenizer uses convert_tokens_to_ids() — not lang_code_to_id (mBART-specific)
        vie_latn_id = tokenizer.convert_tokens_to_ids("vie_Latn")

        # Split text into sentences (NLLB has 512-token input limit)
        ...
        for sentence in sentences:
            ...
            # Pass src_lang per-call to set the source language special token
            input_ids = tokenizer(
                sentence,
                return_tensors="pt",
                src_lang="eng_Latn",
            ...
            output_ids = model.generate(
                **input_ids,
                forced_bos_token_id=vie_latn_id,
