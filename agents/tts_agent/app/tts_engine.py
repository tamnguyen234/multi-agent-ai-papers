import os
import logging
from app.schemas import TTSRequest, TranslateRequest
...
def translate_text(request: TranslateRequest) -> dict:
    """
    Translate English text to Vietnamese using VinAI Translate model.
    """
    env_mode = os.getenv("TTS_MODE", "mock_fallback").lower().strip()
    mode = request.mode.lower().strip() if request.mode else env_mode
    ...
    logger.info(f"Translation execution starting. Mode: '{mode}'")

    if mode == "mock":
        return {
            "translated_text": f"[Bản dịch tiếng Việt từ VinAI - Demo] {request.text}",
            "mode": "mock",
            "fallback_reason": None
        }
        ...
        engine = RealTTSEngine.get_instance()
        translated = engine.translate_en_to_vi(request.text)
        return {
        ...
        try:
            ...
            simplified_error = f"VinAI model translate failed: {e.__class__.__name__}: {str(e)}"
            logger.warning(f"Real translation failed. Falling back to mock. Reason: {simplified_error}")
            return {
                "translated_text": f"[Bản dịch tiếng Việt từ VinAI - Demo (Fallback)] {request.text}",
                "mode": "mock_fallback",
                "fallback_reason": simplified_error
            }
