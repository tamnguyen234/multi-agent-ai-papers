...
# If running in mock_fallback mode, prevent HuggingFace Hub downloads from blocking/hanging the request.
# Set HF_HUB_OFFLINE=1 so it fails immediately if model weights are not cached locally, falling back to mock.
if os.getenv("TTS_MODE", "mock_fallback").lower().strip() == "mock_fallback":
    os.environ["HF_HUB_OFFLINE"] = "1"

from app.schemas import TTSRequest, TTSResponse, TranslateRequest, TranslateResponse
from app.tts_engine import synthesize_text, translate_text
...
def translate_en2vi_endpoint(payload: TranslateRequest):
    """
    Translate English text into Vietnamese using VinAI.
    """
    text = payload.text
    if not text or not text.strip():
        raise HTTPException(
    ...
    if len(normalized_text) > 4000:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Text length exceeds the maximum limit of 4000 characters"
        )
