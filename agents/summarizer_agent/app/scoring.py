...
def score_and_select_top5(papers: List[Dict[str, Any]], limit: int = 5) -> List[Dict[str, Any]]:
    ...
    On Saturday and Sunday, ranks papers strictly by upvote count.
    ...
    for paper in papers:
        if is_weekend:
            # Weekend mode: score is the upvote count
            final_score = float(paper.get("upvotes", 0))
        else:
            # Weekday mode: heuristic trending score
    ...
    if is_weekend:
        # Sort by:
        # 1. Upvotes (score) descending
        # 2. Arxiv ID descending (stable tie-breaker)
    ...
    logger.info(f"Selected top {len(top_papers)} papers using {'upvote count' if is_weekend else 'trending score'}.")
    return top_papers
