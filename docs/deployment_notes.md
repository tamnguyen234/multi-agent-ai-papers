...
```cmd
git clone <repo-url>
cd ai-paper-multi-agent-system
```
...
Mở 5 cửa sổ terminal song song (Backend + 4 Agents), sau đó:
```cmd
...
`run_frontend.bat` hiện dùng `--strictPort` nên sẽ báo lỗi rõ thay vì nhảy sang 5174.
...
## 11. Kết quả kiểm thử (Task 19)

Pipeline test đầy đủ (`scripts/test_full_pipeline.py`) với kết quả:
...
Xem báo cáo chi tiết: [docs/task19_full_pipeline_test.md](task19_full_pipeline_test.md)

---

## 12. Scripts quản lý port (Task Fix)
...
*Tham khảo: [docs/demo_script.md](demo_script.md) | [docs/system_architecture.md](system_architecture.md)*
